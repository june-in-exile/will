...
describe("Token Permit2 Approval Workflow", () => {
  ...
  describe("validateEnvironment", () => {
    ...
    it("should throw error when TESTATOR_PRIVATE_KEY is missing", () => {
      ...
      expect(() => approveTokenPermit2Module.validateEnvironment()).toThrow(
        "Environment variable TESTATOR_PRIVATE_KEY is not set",
      );
    });
  ...
  describe("validateFiles", () => {
    ...
    it("should throw error when will file does not exist", async () => {
      ...
      expect(() => approveTokenPermit2Module.validateFiles()).toThrow(
  ...
  describe("createSigner", () => {
    ...
    it("should create and validate signer successfully", async () => {
      ...
      const signer = await approveTokenPermit2Module.createSigner(
    ...
    it("should handle signer creation failure", async () => {
      ...
      await expect(
        approveTokenPermit2Module.createSigner(privateKey, mockProvider),
  ...
  describe("validateNetwork", () => {
    ...
    it("should validate network connection successfully", async () => {
      ...
      const network =
        await approveTokenPermit2Module.validateNetwork(mockProvider);
    ...
    it("should handle network connection failure", async () => {
      ...
      await expect(
        approveTokenPermit2Module.validateNetwork(mockProvider),
  ...
  describe("readWillData", () => {
    ...
    it("should throw error for invalid JSON", async () => {
      ...
      expect(() => approveTokenPermit2Module.readWillData()).toThrow(
    ...
    it("should throw error for invalid token address", async () => {
      ...
      expect(() => approveTokenPermit2Module.readWillData()).toThrow(
        "Invalid token address in estate 0",
      );
    });
  ...
  describe("extractUniqueTokens", () => {
    ...
    it("should extract unique tokens correctly", () => {
      ...
      const tokenDetails = result.tokenDetails.get(
    ...
    it("should handle single token correctly", () => {
      ...
      const result =
        approveTokenPermit2Module.extractUniqueTokens(singleTokenEstates);
  ...
  describe("getTokenInfo", () => {
    it("should fetch token information successfully", async () => {
      ...
      const result = await approveTokenPermit2Module.getTokenInfo(
        tokenAddress,
        mockSigner,
      );
      ...
      console.log(
        "mockContract calls after execution:",
        mockContract.mock.calls.length,
      );
  ...
  describe("approveToken", () => {
    beforeEach(() => {
      ...
      // Add estimateGas as a property of approve function
      (approveTokenMockContract.approve as any).estimateGas = vi
        .fn()
        .mockResolvedValue(BigInt(50000));
    ...
    it("should approve token successfully", async () => {
      ...
      const result = await approveTokenPermit2Module.approveToken(
        tokenAddress,
        spenderAddress,
        mockSigner,
      );
  ...
  describe("executeTokenApprovals", () => {
    beforeEach(() => {
      ...
      (mockTokenContract.approve as any).estimateGas = vi
        .fn()
        .mockResolvedValue(BigInt(50000));
      mockSigner.getAddress.mockResolvedValue("0xowner");
      mockContract.mockImplementation(() => mockTokenContract);
  ...
  describe("processTokenApproval", () => {
    beforeEach(async () => {
      ...
      process.env.TESTATOR_PRIVATE_KEY =
        "1234567890123456789012345678901234567890123456789012345678901234";
    ...
    it("should handle partial approval failures gracefully", async () => {
      ...
      mockTokenContract.approve.mockImplementation(() => {
        callCount++;
        if (callCount <= 3) {
          // First token calls (estimate gas, approve, wait)
          return {
            hash: "0xabcdef1234567890",
            wait: vi.fn().mockResolvedValue({ status: 1 }),
          };
  ...
  describe("Edge Cases and Error Scenarios", () => {
    ...
    it("should handle duplicate tokens in estates", () => {
      ...
      const { tokens, tokenDetails } =
        approveTokenPermit2Module.extractUniqueTokens(
          duplicateTokenData.estates,
        );
