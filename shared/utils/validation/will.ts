...
import type { FormattedWillData, AddressedWillData, SignedWillData, EncryptedWillData, DownloadedWillData } from "@shared/types/will.js";
import type { Permit2Signature } from "@shared/types/blockchain.js";
import { Estate } from "@shared/types/blockchain.js";

function validateFormattedWill(willData: FormattedWillData): asserts willData is FormattedWillData {
    ...
    willData.estates.forEach((estate: Estate, index: number) => {
        ...
        if (typeof estate.amount === "bigint" && estate.amount <= 0n) {
            throw new Error(`Amount must be greater than zero in estate ${index}: ${estate.amount}`);
        }
    });
}

function validateAddressedWill(willData: AddressedWillData): asserts willData is AddressedWillData {
...
function validateSignedWill(willData: SignedWillData): asserts willData is SignedWillData {
    ...
    const requiredSigFields: (keyof Permit2Signature)[] = ["nonce", "deadline", "signature"];
...
function validateEncryptedWill(willData: EncryptedWillData): asserts willData is EncryptedWillData {
    ...
    const base64Fields: (keyof Pick<DownloadedWillData, "iv" | "authTag" | "ciphertext">)[] = [
...
function validateDownloadedWill(willData: DownloadedWillData): asserts willData is DownloadedWillData {
    validateEncryptedWill(willData);
}

export { validateFormattedWill, validateAddressedWill, validateSignedWill, validateEncryptedWill, validateDownloadedWill }