...
class ContractAddressExtractor {
    ...
    extract(contractName: string, chainId: string): string {
        try {
            ...
            const broadcastData: BroadcastData = JSON.parse(readFileSync(broadcastPath, 'utf8'));
            
            // Priority: Extract from receipts
            const contractAddressFromReceipts = this.extractFromReceipts(broadcastData);
            if (contractAddressFromReceipts) {
                return contractAddressFromReceipts;
            }

            // Fallback: Extract from transactions
            const contractAddressFromTransaction = this.extractFromTransactions(broadcastData);
            ...
            console.error(chalk.red(`Error processing broadcast file: ${errorMessage}`));
            throw error;
        }
    }
...
async function main(): Promise<void> {
    ...
    if (args.length < 3) {
        console.error(chalk.red(`❌ Usage: pnpm exec tsx ${process.argv[1]} <KEY> <CONTRACT_NAME> <CHAIN_ID>`));
        console.error(chalk.gray(`📝 For example: pnpm exec tsx ${process.argv[1]} TESTAMENT_FACTORY_ADDRESS TestamentFactory 31337`));
        process.exit(1);
    }
