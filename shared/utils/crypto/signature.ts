...
function validateMessage(message: string): boolean {
    ...
    if (message.length > SIGNATURE_CONFIG.maxMessageLength) {
        throw new Error(`Message too long: ${message.length} characters (max: ${SIGNATURE_CONFIG.maxMessageLength})`);
    }
...
function validatePrivateKey(privateKey: string): string {
    ...
    if (cleanKey.length !== SIGNATURE_CONFIG.privateKeyLength) {
        throw new Error(`Invalid private key length: expected ${SIGNATURE_CONFIG.privateKeyLength} characters, got ${cleanKey.length}`);
    }
    ...
    if (cleanKey === '0'.repeat(SIGNATURE_CONFIG.privateKeyLength) ||
...
function validateSignature(signature: string): boolean {
    ...
    if (signature.length !== SIGNATURE_CONFIG.signatureLength) {
        throw new Error(`Invalid signature length: expected ${SIGNATURE_CONFIG.signatureLength} characters, got ${signature.length}`);
    }
