...
function validateInput(input: HashableInput): string {
    ...
    if (sizeInBytes > HASH_CONFIG.maxInputSize) {
        throw new Error(`Input too large: ${sizeInBytes} bytes (max: ${HASH_CONFIG.maxInputSize} bytes)`);
    }
...
function validateEncoding(message: string, encoding: string = 'utf8'): Buffer {
    try {
        ...
        if (encoding.toLowerCase().includes('utf') && decoded !== message) {
            console.warn(chalk.yellow('⚠️ Warning: UTF-8 encoding/decoding cycle mismatch detected'));
        }
...
function validateHashOutput(hash: string): boolean {
    ...
    if (hash.length !== HASH_CONFIG.expectedHashLength) {
        throw new Error(`Invalid hash length: expected ${HASH_CONFIG.expectedHashLength}, got ${hash.length}`);
    }

    if (!HASH_CONFIG.hashPattern.test(hash)) {
        throw new Error('Hash does not match expected pattern (0x followed by 64 hex characters)');
    }
...
export function keccak256Bytes(bytes: ByteInput): string {
    try {
        ...
        if (bytes instanceof Uint8Array) {
            ...
            throw new Error('Bytes input must be Uint8Array, ArrayBuffer, Buffer, or number array');
        }
        ...
        if (byteArray.length > HASH_CONFIG.maxInputSize) {
            throw new Error(`Byte array too large: ${byteArray.length} bytes (max: ${HASH_CONFIG.maxInputSize} bytes)`);
        }
...
export function keccak256(input: HashableInput, encoding: string = 'utf8'): string {
    try {
        ...
        if (HASH_CONFIG.enableLogging) {
            const inputPreview = message.length > 50
    ...
    try {
        ...
        if (typeof lastArg === 'string' && HASH_CONFIG.supportedEncodings.includes(lastArg.toLowerCase())) {
            ...
            if (actualInputs.length === 0) {
                throw new Error('At least one input is required besides encoding parameter');
            }
        }
