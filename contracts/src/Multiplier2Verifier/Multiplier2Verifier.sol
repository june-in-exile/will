...
contract VerifierConstants1 is IVerifierConstants {
    ...
    function getBatchIC(uint256 startIdx, uint256 count)
        ...
        returns (uint256[] memory xs, uint256[] memory ys)
    {
        (uint256 rangeStart, uint256 rangeEnd) = _getICRange();
        uint256 adjustedStart = startIdx + rangeStart;
        if (adjustedStart < rangeStart || adjustedStart + count > rangeEnd) {
            revert IndexOutOfRange(startIdx);
        }
        ...
        for (uint256 i = 0; i < count; i++) {
            (xs[i], ys[i]) = _getIC(adjustedStart + i);
        }
    }
}
