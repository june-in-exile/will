 ...
 */
template PaddedCtrEncrypt(keyBits, plaintextBytes) {
    var Nk;
    assert(keyBits == 128 || keyBits == 192 || keyBits == 256);
    if (keyBits == 128) {
        Nk = 4;
    ...
    var numBlocks = plaintextBytes \ 16 + 1;
    var paddingBytes = 16 - plaintextBytes % 16;
    ...
    // Generate incremented (for last 4 bytes only) counters for each block
    for (var i = block; i < numblocks; block++) {
