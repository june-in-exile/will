...
template ThetaStateArray() {
    ...
    for (var x = 0; x < 5; x++) {
        for (var z = 0; z < 64; z++) {
            ...
            columnSums[x][z] <== Sum(5)(columnValues[x][z]);
        }
    }
    
    signal twoColumnSums[5][64];
    for (var x = 0; x < 5; x++) {
        for (var z = 0; z < 64; z++) {
            twoColumnSums[x][z] <== columnSums[(x + 4) % 5][z] + columnSums[(x + 1) % 5][(z + 63) % 64];
        }
    }

    signal stateArraySums[5][5][64];
    for (var x = 0; x < 5; x++) {
        for (var y = 0; y < 5; y++) {
            for (var z = 0; z < 64; z++) {
                stateArraySums[x][y][z] <== stateArray[x][y][z] + twoColumnSums[x][z];
                newStateArray[x][y][z] <== Mod2()(stateArraySums[x][y][z]);
            }
        }
    }
}