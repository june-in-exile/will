 ...
 */
template Theta() {
    signal input state[25];  // 5x5 state matrix, flattened (row-major)
    signal output newState[25];
    
    // Column parity computation: C[x] = state[x,0] ⊕ state[x,1] ⊕ state[x,2] ⊕ state[x,3] ⊕ state[x,4]
    signal C[5];    
    signal columnValues[5][5];
    for (var x = 0; x < 5; x++) {
        for (var y = 0; y < 5; y++) {
