...
const TOKEN_PERMISSIONS_TYPEHASH = hexToByte(
  "0x618358ac3db8dc274f0cd8829da7e234bd48cd73c4a740aede1adec9846d06a1",
) as Byte32;
const PERMIT_BATCH_TRANSFER_FROM_TYPEHASH = hexToByte(
...
describe("AbiEncode Circuit", function () {
  ...
  describe("Abi.Encode(PERMIT_BATCH_TRANSFER_FROM_TYPEHASH, permissionDigest, spender, nonce, deadline)", function (): void {
    beforeAll(async function (): Promise<void> {
      ...
      circuit.setConstraint(
        "abi.encode(PERMIT_BATCH_TRANSFER_FROM_TYPEHASH, permissionDigest, spender, nonce, deadline)",
      );
    });
    ...
    it("should encode batch permit correctly", async function (): Promise<void> {
      const tokenPermissions = hexToByte(
        "0x53fdab7265e432f9ca14c873ec6fa8dcbd18fdedcb42a1602cad79aa9e1b5ec9",
      );
      const spender = bigIntToByte(
        732565455009656814900284710031916618803287670512n,
        32,
      );
