...
export async function generateTestTemplate(
    ...
    if (!template) {
        throw new Error(`Template "${templateName}" not found in ${absoluteCircuitPath}`);
    }

    // Create test directory and file
    const testDir = createTestDir(absoluteCircuitPath);
    const testPath = path.join(testDir, `${templateName}.circom`);
    const testContent = generateTestContent(template, absoluteCircuitPath, testDir);
...
function generateTestContent(template: Template, circuitPath: string, testDir: string): string {
...
function generateTemplateInstantiation(template: Template): string {
    const paramsStr = template.params.length > 0 ? `(${template.params.join(', ')})` : '()';
    const hasTaggedInputs = template.inputs.some(input => input.tag && !input.busType);

    // Prepare input arguments
    const inputArgs = template.inputs.map(input =>
