import { compileToWasm, WitnessTesterClass } from "./utils";
const circom_tester = require("circom_tester");

describe("Modulo Circuit", function () {
  let circuit: WitnessTesterClass<['in', 'modulus'], ['quotient', 'remainder']>;

  describe("6-bit Modulo Operations", function (): void {
    beforeAll(async function (): Promise<void> {
      const circomWasmTester = await compileToWasm("./shared/components/mod.circom", {
        templateParams: ["6"],
      });

      circuit = new WitnessTesterClass(circomWasmTester);
    });
